...
"""
Module Modèles pour la gestion des Projets de Fin d'Études (PFE).

Ce module contient les modèles pour gérer:
- Les paramètres globaux des PFEs (plafonds)
- Les salles de soutenance
- Les rapporteurs
- Les PFEs eux-mêmes
- Les étudiants assignés aux PFEs
- Les soutenances et résultats
"""
...
class Salle(models.Model):
    ...
    Les PFEs sont soutenus dans ces salles selon un calendrier établi.
    """
...
class Rapporteur(models.Model):
    """
    Modèle représentant un rapporteur (examinateur) de PFE.
    
    Les rapporteurs évaluent les travaux des étudiants pendant les soutenances.
    Ils ont une limite de groupes qu'ils peuvent examiner (plafond_groupes).
    
    Attributs similaires à un enseignant:
    - Données personnelles (matricule, CIN, nom, prénom)
    - Coordonnées (email, téléphone)
    - Grade et informations administratives
    """
...
class PFE(models.Model):
    """
    Modèle représentant un Projet de Fin d'Études (PFE).
    
    Un PFE:
    - A un sujet et une durée
    - Est encadré par un enseignant
    - Regroupe 1 ou plusieurs étudiants
    - Peut avoir des documents associés (convention, lettre d'affectation)
    - Est affecté à un lieu de stage
    """
